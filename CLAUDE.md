...
`fugo doctor` checks the toolchain (Go / Flutter / git / protoc / gofumpt) and, when run inside a project, imports `fugo.toml` and validates the project (structure, resolved fugo module, Flutter client built, configured addr free, and `go build ./...`); it exits non-zero on a blocking issue. The Go process finds the Flutter binary via `FUGO_FLUTTER_BINARY`, then by searching up the tree for the fugo repo (see `findFlutterBinary` in `app.go` / `cmd/fugo-spike/main.go`). The gRPC address is `FUGO_ADDR` (default `127.0.0.1:9510`). The active `fg.Theme` is forwarded to the client as `FUGO_THEME_SEED` (primary color hex) + `FUGO_THEME_BRIGHTNESS` (`light`/`dark`), which seed the client's **Material 3** `ColorScheme.fromSeed` — **light by default** (call `fg.UseTheme(fg.DarkTheme())` before `RunStandalone` to change it).
