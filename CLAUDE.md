...
| `cmd/fugo/` | The `fugo` CLI: `init`, `run` (hot-reloads by default; `--no-watch` to disable), `build`, `doctor` (toolchain + project coherence; `--fix`), `widgets`, `upgrade` (self-update via `go install …@latest`). |
