...
cd myapp && fugo run           # go build → launch app → spawn Flutter; hot-reloads on .go changes by default (--no-watch to disable)
```
...
`fugo doctor` checks the toolchain (Go / Flutter / git / protoc / gofumpt) and, when run inside a project, imports `fugo.toml` and validates the project (structure, resolved fugo module, **coherence** between `go.mod`'s module and `main.go`'s `<module>/ui` import + a `ui.Build`, Flutter client built, configured addr free, and `go build ./...`); it exits non-zero on a blocking issue. `fugo doctor --fix` first repairs the auto-fixable parts (writes `fugo.toml`, `git init`, `go mod tidy`). The Go process finds the Flutter binary via `FUGO_FLUTTER_BINARY`, then by searching up the tree for the fugo repo (see `findFlutterBinary` in `app.go` / `cmd/fugo-spike/main.go`). The gRPC address is `FUGO_ADDR` (default `127.0.0.1:9510`). The active `fg.Theme` is forwarded to the client as `FUGO_THEME_SEED` (primary color hex) + `FUGO_THEME_BRIGHTNESS` (`light`/`dark`), which seed the client's **Material 3** `ColorScheme.fromSeed` — **light by default** (call `fg.UseTheme(fg.DarkTheme())` before `RunStandalone` to change it).
...
| `cmd/fugo/` | The `fugo` CLI: `init`, `run` (hot-reloads by default; `--no-watch` to disable), `build` (also writes Linux packaging — `.desktop`, `install.sh`, `PKGBUILD` — next to the binary, see `autostart.go`'s neighbor `writeLinuxPackaging`), `doctor` (toolchain + project coherence; `--fix`), `autostart` (`enable`/`disable` — XDG autostart entry on Linux, `Run` registry key on Windows; see `autostart.go`), `widgets`, `upgrade` (self-update via `go install …@latest`). |
...
- **Versioning tracks Flutter, on purpose.** `VERSION` is `<flutter-version>-fugo.<build>` (e.g. `3.44.8-fugo.0`) — the first three numbers are always the exact Flutter SDK version the precompiled client and `flutter_client/` are built against, so there is never ambiguity between a fugo release and the Flutter it renders with. `FLUTTER_VERSION` (plain `X.Y.Z`, no suffix) is the single source of truth for that pin, and `fugo doctor` warns (doesn't fail — the Flutter SDK is optional, see below) if your installed Flutter doesn't match it.
- **The whole release pipeline is automatic — nothing to tag or publish by hand.** Three workflows chain together: `check-flutter-version.yml` runs weekly, and when a newer stable Flutter exists it first *validates* it for real (`flutter build linux --release` against `flutter_client/`) before touching anything — on success it bumps `FLUTTER_VERSION` + `VERSION` (fresh `-fugo.0`) + `CHANGELOG.md` and pushes straight to `main` (on failure it opens an issue instead and changes nothing). Any push to `main` that changes `VERSION` — whether from that automation or from `make release`/`make release-flutter` — is picked up by `tag-release.yml`, which creates and pushes the `v<version>` tag (skips if it already exists). That tag push triggers `release-flutter-client.yml`, which builds the precompiled client for Linux/Windows pinned to `flutter-version-file: FLUTTER_VERSION` and publishes it to the GitHub Release. End to end: a new Flutter stable release → validated → committed → tagged → built → published, with zero manual steps. The only human-gated part is a genuinely broken Flutter upgrade (validation build fails → issue, not a commit).
- **Releases:** never hand-edit `VERSION`, `FLUTTER_VERSION`, or `CHANGELOG.md`. Two paths: `make release MSG="..."` bumps only the `-fugo.N` build suffix (a fugo-only fix against the same Flutter version); `make release-flutter FLUTTER=<version> MSG="..."` bumps the targeted Flutter version itself (resets to `-fugo.0`). Both update CHANGELOG and commit — just `git push origin main` after; tagging and publishing happen automatically (see above), there is no `git tag` step anymore. There is no `TYPE=patch|minor|major` either — major/minor don't mean anything under this scheme. Pre-push hook fails if VERSION lacks a matching CHANGELOG entry.
...
- **Linux**: works on any distro with GTK3 (Arch, Ubuntu, etc. — nothing distro-specific, the CMake build only needs `gtk+-3.0` via pkg-config). **Wayland/Hyprland is confirmed working**, but through **XWayland**, not Flutter's native Wayland path — the Flutter Linux embedder is GTK-based and, as of the pinned Flutter version, relies on X11; `flutter_client/linux/runner/my_application.cc` (Flutter's standard template) only special-cases X11 explicitly and just assumes Wayland "will work" for its header-bar decision, with no Fugo-specific Wayland logic. Verified empirically in a live Hyprland/Wayland session (`XDG_SESSION_TYPE=wayland`, `XDG_CURRENT_DESKTOP=Hyprland`) with `Xwayland :0 -rootless` running: `fugo-spike` built, launched, rendered, resized, and shut down cleanly. If a future Flutter release drops XWayland fallback or a compositor doesn't run one, this would break — there is no native-Wayland-specific code path to fall back on.
- **Windows**: supported, see the Commands section above (`flutter build windows`, TCP transport instead of UDS).
- **Android/iOS/mobile: explicitly out of scope**, not a "todo" — the architecture doesn't fit. Fugo's model is a Go binary spawned as a subprocess by the Flutter client (`supervisor.StartFlutter`), talking over a Unix domain socket (Linux/macOS) or TCP (Windows). Mobile OSes sandbox apps and don't allow spawning arbitrary subprocesses or opening raw UDS the way this needs; supporting mobile would mean embedding the Go runtime in-process (e.g. via `gomobile`/cgo bindings) instead of spawning it, which is a different architecture, not an incremental change. There is no `flutter_client/android/` or `ios/` directory.
...
- `ROADMAP/` (12 files) and `docs/` are the real design specs and are mostly **in Spanish**. `SPEC.md` is the detailed specification. `docs/FUGO_IDIOMATICO.md` is a from-scratch, code-verified guide (also in Spanish) to the whole framework — architecture, the full widget catalog, theming, state management (closures vs. the opt-in `fg.Store[S]`), host services, platform support, and packaging; prefer it over ROADMAP/SPEC when they conflict, since it was written against the current code rather than the original design docs.
